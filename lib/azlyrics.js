...
exports.getURL = query => {
	return new Promise((resolve, reject) => {
		fetch(`https://search.azlyrics.com/search.php?q=${query}`, 
		...
		.then(body => {
			...
			if($('div[class="panel"]').length === 1) {
				return $('td[class="text-left visitedlyr"] a').attr('href') 
					? resolve($('td[class="text-left visitedlyr"] a').attr('href').valueOf()) 
...
exports.getLyrics = link => {
	return new Promise((resolve, reject) => {
		...
		.then(body => {
			const $ = cheerio.load(body);
			const lyrics = $('div[class="col-xs-12 col-lg-8 text-center"] div').eq(6).text() 
				? $('div[class="col-xs-12 col-lg-8 text-center"] div').eq(6).text().trim() 
				: null;
			return lyrics ? resolve(lyrics) : reject(new Error('I couldn\'t find the lyrics!'));
		});
	});
};
